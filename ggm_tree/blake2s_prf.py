...
# cpu implementation
# BLAKE2s implementation mirrors the GPU kernel below
_MASK32 = 0xFFFFFFFF
...
def _G(v: list, a: int, b: int, c: int, d: int, mx: int, my: int) -> None:
    """
    G mixing function (RFC 7693 section 3.1), mutates v in place
    """
...
def _compress(h: list, block: bytes, t_lo: int, t_hi: int, is_last: bool) -> None:
    """
    compress one 64-byte block into state h (mutated in place)
    """
...
def _blake2s_keyed_16(key: bytes, data_byte: int) -> bytes:
    """
    keyed BLAKE2s of one byte with a 16 byte key, 16 byte digest
    written from scratch per RFC 7693, mirrors the GPU kernel exactly
    """

    h = _BLAKE2S_IV[:]
    h[0] ^= 0x01010000 | (16 << 8) | 16

    _compress(h, bytes(key) + b"\x00" * 48, 64, 0, False)
    _compress(h, bytes([data_byte]) + b"\x00" * 63, 65, 0, True)

    return b"".join(h[i].to_bytes(4, "little") for i in range(4))
...
def blake2s_prf_cpu(key: bytes) -> tuple:
    ...
    left  = _blake2s_keyed_16(key, 0x00)
    right = _blake2s_keyed_16(key, 0x01)
    return left, right
